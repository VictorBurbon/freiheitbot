...
async def help(ctx): 
    ...
    emb.add_field(name= "Информация про команды", value= "`/help`")
    emb.add_field(name= "Проверить ping", value= "`/ping`")
    emb.add_field(name= "Игра bibametr", value= "`/bibametr`")
    emb.add_field(name= "Игра Coin", value= "`/coin`")
    emb.add_field(name= "Посмотреть аватарку юзера", value= "`/avatar @username`")
    emb.add_field(name= "Игра 8-ball", value= "`/ball`")
    emb.add_field(name= "Правила сервера", value= "`/rules`")
    emb.add_field(name= "Информация о юзере", value= "`/info @username`")
...
async def cmd(ctx):
    ...
    emb = discord.Embed(title= "GIVEAWAY начат", description=f" {role.mention} **Для участия нажмите на реакцию ниже. Три победителя получат по steam-ключу игры Little Nightmares.**", color=0x99ff99)
